...
import { formatWeatherLabel, teamNameFor, titleCase, type TeamNames } from "./labels";
...
const formatKickoffEventDetails = (payload: unknown, teamNames: TeamNames): { baseText: string; detailText?: string } | undefined => {
  ...
  if (!kickoff.kickoffKey) return { baseText: `Kick-off · ${formatKickoffLabel(payload)}` };

  if (kickoff.kickoffKey === "CHANGING_WEATHER") {
    const weather = kickoff.details?.newWeather;
    return { baseText: "Kick-off · Weather", detailText: weather ? formatWeatherLabel(String(weather)) : undefined };
  }

  if (kickoff.kickoffKey === "TIME_OUT") {
    const delta = kickoff.details?.appliedDelta;
    return { baseText: "Kick-off · Time-Out", detailText: delta === 1 || delta === -1 ? `${delta > 0 ? "+" : ""}${delta} Turn` : undefined };
  }
  ...
  if (kickoff.kickoffKey === "THROW_A_ROCK") {
    ...
    return { baseText: "Kick-off · Rock", detailText: detailParts.length ? detailParts.join(" ") : undefined };
  }
  ...
  if (kickoff.kickoffKey === "PITCH_INVASION") {
    ...
    return { baseText: "Kick-off · Pitch Invasion", detailText: [affectedA, affectedB, notes].filter(Boolean).join(" ") || undefined };
  }
...
export function formatEventText(event: MatchEvent, teamNames: TeamNames): string {
  ...
  if (type === "touchdown") return `Touchdown · ${teamNameFor(event.team, teamNames)} · Player ${playerId(event.payload?.player)}`;
  if (type === "completion") return `Completion · ${teamNameFor(event.team, teamNames)} · Player ${playerId(event.payload?.passer)}`;
  if (type === "interception") return `Interception · ${teamNameFor(event.team, teamNames)} · Player ${playerId(event.payload?.player)}`;
  ...
  if (type === "injury") {
    ...
    return `${team} Player ${id} · Casualty: ${result}${formatApothecaryOutcome(event.payload)}`;
  }
  ...
  if (type === "weather_set") return withDetail("Weather changed", event.payload?.weather ? formatWeatherLabel(String(event.payload.weather)) : undefined);
  ...
  if (type === "half_changed") return withDetail("Half changed", typeof event.payload?.half === "number" ? `Half ${event.payload.half}` : undefined);
